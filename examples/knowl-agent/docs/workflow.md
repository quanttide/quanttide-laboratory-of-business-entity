...
本体质量的唯一标准：**换一套完全不同的文档，这个 pattern 是否仍然适用？** 如果 pattern 中包含只在一份文档中出现过的角色名、阶段名、数字、文件引用，说明抽象不到位。

**操作步骤**

1. **跨文件收集证据**：在写 pattern 之前，先找至少 2-3 个来源，确认这个结构是反复出现的，不是单份文档的特例。
2. **识别常量与变量**：pattern 中哪些是固定结构（常量），哪些是可填充的具体值（变量）。常量留在 pattern，变量写到 instances。
3. **写 pattern**：用 `<变量>` 标记可填充的槽位。例如「<角色> 以 <职责列表> 和 <权限列表> 成对定义」而非「设三类核心岗位：项目经理…」。
4. **自检**：
   - **跨文件检查**：这个结构是否出现在多个源文件中？如果只出现在一个文件里，它可能不是本体。
   - **去值测试**：把 pattern 中所有角色名、阶段名、数字、条款号删掉后，剩余的句子是否仍在描述一种结构？
   - **换源测试**：换一批完全不同的文档，这个 pattern 能否直接套用？
