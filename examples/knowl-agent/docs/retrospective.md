...
| 问题 | 方案 |
|------|------|
| JSON 自修复 | 去掉。改由 validate.sh 报错 → AI 手动修 | ✅ 已修 |
| jq 流坑 | 任何 jq 表达式涉及 `//` 时，先用 `|` 限域 | ✅ 已修 |
| 端到端测试 | 加 `scripts/test-pipeline.sh`，完整跑一遍所有脚本 | 待做 |
| 文件保护 | 所有写操作经临时文件 + 合法性验证 | ✅ 已修（去掉写操作） |
...
这违反了 `decision-guide.md` 定义的"脚本只暴露信号，不代人做决定"原则。stub 创建更适合由 AI 在解读检测结果时按需生成，而不是脚本运行时直接写文件。
