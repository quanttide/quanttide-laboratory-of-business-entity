...
def list_positions(
    q: str | None = Query(None, min_length=1, description="搜索：岗位名称/类型"),
    ...
    if q:
        like = f"%{q}%"
        qb = qb.filter(or_(Position.name.ilike(like), Position.type.ilike(like)))
    if type:
        qb = qb.filter(Position.type == type)
    if active is not None:
        qb = qb.filter(Position.active == active)
    return qb.order_by(Position.created_at.desc()).offset(skip).limit(limit).all()
